...
void mtpWorker() {
  ...
  if( g_stopRequested.load() || g_stopping.load() ) {
    ...
    try {
      ...
      LOG( ERROR ) << "MTP responder stopped after an error: " << e.what() << std::endl;
      setStatus( "MTP stopped after filesystem error." );
    }
    catch( ... ) {
      LOG( ERROR ) << "MTP responder stopped after an unknown error." << std::endl;
      setStatus( "MTP stopped after filesystem error." );
    }
  }
