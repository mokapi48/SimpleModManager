...
static bool deleteRecursive(const char* path) {
    ...
    bool ok = true;
    if (pathLength >= sizeof(pathbuf) - 2) {
    ...
    size_t pathRemaining = sizeof(pathbuf) - pathLength - 1;
    ...
    while ((entry = readdir(dir))) {
        ...
        if (entry->d_type == DT_UNKNOWN) {
            ...
            isDir = stat(pathbuf, &childStat) == 0 && S_ISDIR(childStat.st_mode);
        }

        if (isDir) {
            ok = deleteRecursive(pathbuf) && ok;
            if (rmdir(pathbuf) != 0 && errno != ENOENT)
                ok = false;
        } else {
...
MtpResponseCode MtpServer::doMoveObject() {
    ...
    return mDatabase->moveFile(handle, newparent);
}
